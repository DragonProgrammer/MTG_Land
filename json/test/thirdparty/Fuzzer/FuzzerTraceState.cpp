...
{
    DoingMyOwnMemmem = true;
}
ScopedDoingMyOwnMemmem::~ScopedDoingMyOwnMemmem()
{
    DoingMyOwnMemmem = false;
}
...
class TraceState
{
  public:
        ...
        for (size_t i = 0; i < NumMutations; i++)
        {
            ...
            if (Options.Verbosity >= 2)
            {
                ...
                if ((AutoDictAdds & (AutoDictAdds - 1)) == 0)
                {
                    ...
                    std::sort(CountedUnits.begin(), CountedUnits.end(), [](const CU& a, const CU& b) {
    ...
    {
        if (!RecordingMemmem || !F->InFuzzingThread())
            return;
        if (Size <= 1)
            return;
...
int TraceState::TryToAddDesiredData(uint64_t PresentData, uint64_t DesiredData, size_t DataSize)
{
    if (NumMutations >= kMaxMutations || !WantToHandleOneMoreMutation())
        return 0;
...
void TraceState::TraceMemcmpCallback(size_t CmpSize, const uint8_t* Data1, const uint8_t* Data2)
{
    if (!RecordingMemcmp || !F->InFuzzingThread())
        return;
    ...
    if ((Added1 || Added2) && Options.Verbosity >= 3)
    {
        ...
        if (Added1)
            PrintASCII(Data1, CmpSize);
        if (Added2)
            PrintASCII(Data2, CmpSize);
        Printf("\n");
    }
}

void TraceState::TraceSwitchCallback(uintptr_t PC, size_t ValSizeInBits, uint64_t Val, size_t NumCases, uint64_t* Cases)
{
    if (F->InFuzzingThread())
        return;
